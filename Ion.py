...
from interface.base import TypeReflectBaseModel
...
class Ion(TypeReflectBaseModel):
    ...
    def select_levels(self, selected_levels):
        ...
        if selected_levels:
            ...
            for l, (manifold, m_F, alias) in enumerate(selected_levels):
                ...
                self.state[alias] = basis(self.N_levels, self.N_levels-1- l) # ranges from 0 to self.N_levels-1
            ...
            for alias1 in self.levels.keys():
                for alias2 in self.levels.keys():
                    if alias1 == alias2 or (alias1, alias2) in alias_pairs or (alias2, alias1) in alias_pairs:
                        continue
                    alias_pairs.append((alias1, alias2),)
            ...
            for alias1, alias2 in alias_pairs:
                ...
                manifold1, manifold2 = self.alias_to_manifold[alias1], self.alias_to_manifold[alias2]
    ...
    def level(self, name):
        "Wrapper function: name is either an alias or label e.g. 's' or 'S1/2, respectively"
