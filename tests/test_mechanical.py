...
class TestMechanical:
    def test_trapped_ions_equilibrium_position_polynomial(self):
        ...
        np.testing.assert_allclose(x_ep, reference_x_ep)
    ...
    def test_trapped_ions_normal_modes_polynomial(self):
        ...
        expected_x_pa = np.array(
            [
                [1.00000000e00, 1.34023762e-19, 1.00634804e-17],
                [-1.38689859e-19, 1.00000000e00, 5.68049658e-17],
                [-9.87275272e-18, 1.49233460e-17, 1.00000000e00],
            ]
        )
        ...
        np.testing.assert_allclose(b_pa, expected_b_pa)
    ...
    def test_trapped_ions_polynomial_potential_fit(self):
        ...
        expected_x_pa = np.eye(3)
        expected_w_pa = np.array([2513274.122872, 2462495.668113, 2389593.742409, 2261946.710585,
              2205389.085922, 2123679.438755, 1210553.178732,  870623.727631,
               502654.824575])

        np.testing.assert_allclose(x_pa, expected_x_pa, atol=1e-8)
        np.testing.assert_allclose(w_pa, expected_w_pa)
