...
class Chamber:
    ...
    def __init__(self, chain, B, Bhat, lasers):
        ...
        for ion in self.chain.ions:
            for alias in ion.levels:
                ...
                level.energy += zeeman_energy_shift(level, self.B) / (
                    cst.hbar * 2 * np.pi
                )
    ...
    def set_laser_intensity_from_pi_time(self, laser_index, pi_time, transition):
        ...
        I = (
            ...
            * cst.c
            / 2
            * ((cst.hbar * np.pi) / (pi_time * matrix_elem * cst.e)) ** 2
        )
    ...
    def set_laser_intensity_from_rabi_frequency(
        self, laser_index, rabi_frequency, transition
    ):
        ...
        intensity_fn = self.set_laser_intensity_from_pi_time(
            laser_index, pi_time, transition
        )
    ...
    def rabi_frequency_from_intensity(self, laser_index, intensity, transition):
        ...
        matrix_elem = self.compute_matrix_element(laser, transition)
        E = sqrt(
            2 * intensity / (cst.eps_0 * cst.c)
        )  # Formula for electric field magnitude given intensity
    ...
    def compute_matrix_element(self, laser, transition):
        ...
        (
            ...
            lvl1.spin_orbital_nuclear_magnetization,
            lvl2.spin_orbital_nuclear_magnetization,
        )
        ...
        if not np.array_equal(self.Bhat, np.array([0, 0, 1])):
            ...
            amatrix = np.array([[0, -a[2], a[1]], [a[2], 0, -a[0]], [-a[1], a[0], 0]])
            R = (
                np.identity(3)
                + sin(theta) * amatrix
                + 2 * sin(theta / 2) ** 2 * np.matmul(amatrix, amatrix)
            )
        ...
        if transition.multipole == "E1":
            ...
            units_term = sqrt(
                (3 * np.pi * cst.eps_0 * cst.hbar * cst.c**3) / omega_0 * A
            ) / (
                omega_0 * cst.e
            )  # <- anomalous constants I needed to add... hmm
            hyperfine_term = sqrt((2 * F2 + 1) * (2 * F1 + 1)) * wigner_6j(
                J1, J2, 1, F2, F1, I
            )
            ...
            geometry_term = (
                sqrt(2 * J2 + 1)
                * polarization_map[q].dot(eps_hat)
            ...
            units_term = sqrt(
                (15 * np.pi * cst.eps_0 * cst.hbar * cst.c**3) / omega_0 * A
            ) / (
                omega_0 * cst.e
            )  # <- anomalous constants I needed to add... hmm
            hyperfine_term = sqrt((2 * F2 + 1) * (2 * F1 + 1)) * wigner_6j(
            ...
            # q -> polarization
            polarization_map = {
                -2: 1 / sqrt(6) * np.array([[1, 1j, 0], [1j, -1, 0], [0, 0, 0]]),
                -1: 1 / sqrt(6) * np.array([[0, 0, 1], [0, 0, 1j], [1, 1j, 0]]),
            ...
            geometry_term = (
                sqrt(2 * J2 + 1)
                * k_hat.dot(np.matmul(polarization_map[q], eps_hat))
