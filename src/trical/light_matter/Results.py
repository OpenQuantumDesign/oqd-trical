...
class Results():
    ...
    def quick_plot(self):
        ...
        y_min = min([min(self.expectation_values[name]) for name in self.expectation_values])
        y_max  = max([max(self.expectation_values[name]) for name in self.expectation_values])
