...
def construct_H(
    chamber, timescale, ld_order=1, rwa_cutoff="inf", boost_freq=None, sym_rep=False
):
    """
            ...
            for n, ion in enumerate(ions):
                ...
                for transition in ion.transitions:
                    ...
                    laser_freq = (
                        2
                        * np.pi
                        * (cst.c / laser.wavelength + laser.detuning)
                        * timescale
                    )
                    ...
                    sigma = tensor(
                        *int_buffer, *[qeye(mode.N) for mode in chain.selected_modes]
                    )
                    ...
                    Delta_nmkj_sym = (
                        f"\left(\omega_{m+1} - "
                        + f"\omega^{{{f'{lbl1},{lbl2}'}}}_{{{f'0,{n+1}'}}}"
                        + r"\right)"
                    )
                    exp_arg_sym = (
                        f"-i\left[{{{Delta_nmkj_sym} t - \phi_{m+1}}}" + r"\right]"
                    )
                    int_term_sym = (
                        rf"|{lbl2}\rangle \langle {lbl1}|" + f"e^{{{f'{exp_arg_sym}'}}}"
                    )
                    ...
                    for l, mode in enumerate(chain.selected_modes):
                        ...
                        nu = (
                            2 * np.pi * mode.eigenfreq * timescale
                        )  # mode eigenfrequency

                        eta_nml = lambdicke(mode, ion, laser)

                        alpha = (
                            1j * eta_nml * exp(1j * nu * t)
                        ...
                        mot_buffer[l] = mode.displace(
                            alpha, ld_order, rwa_cutoff, nu, Delta_nmkj
                        )

                        displace = tensor(
                            *[qeye(J_n) for _ in range(len(ions))], *mot_buffer
                        )
                        mot_term *= displace
                    ...
                    if boost_freq:
                        ...
                        pauli_z = tensor(
                            *pz_int_buffer,
                            *[qeye(mode.N) for mode in chain.selected_modes],
                        )
