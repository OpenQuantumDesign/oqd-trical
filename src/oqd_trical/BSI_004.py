...
trical_cache = {
    'N': None, 'wx': None, 'wy': None, 'wz': None,
    'freqs_x': None, 'freqs_y': None, 'freqs_z': None, 
    'evecs_x': None, 'evecs_y': None, 'evecs_z': None,
    'x_eq': None, 'raw_z_eq': None, 
    'trical_stable': True, 'steane_stable': True, 'w_crit': 0.0,
...
def solve_trical(N, wx_mhz, wy_mhz, wz_mhz):
    if (trical_cache['N'] == N and 
        trical_cache['wx'] == wx_mhz and 
        trical_cache['wy'] == wy_mhz and 
    ...
    try:
        if oqd_trical is None: raise Exception("oqd_trical not installed")
        ...
        l_0 = ((scipy_cst.e**2 / (4 * np.pi * scipy_cst.epsilon_0)) / (MASS_ION * omega_z_si**2))**(1/3)
...
for ax, label, color in zip(axes, labels, colors):
    ...
    lines[label] = ax.plot([], [], '|', color=color, markersize=30, markeredgewidth=1.5)[0]
    highlights[label] = ax.plot([], [], 'o', color='gold', markeredgecolor='black', markersize=12, zorder=10)[0]
...
ax_status.axis('off') 
status_text = ax_status.text(0.0, 0.5, "STATUS: INITIALIZING...", ha='left', va='center', fontsize=11, fontweight='bold', color='gray')
...
def animate(frame):
    try:
        ...
        freqs_z, freqs_x, freqs_y = trical_cache['freqs_z'], trical_cache['freqs_x'], trical_cache['freqs_y']
        ...
        if trical_cache['table_drawn_for'] != current_state:
            ax_table.clear()
            ax_table.axis('off')
            ax_table.set_title("Normal Modes & Z-Positions", fontweight='bold', pad=10)
            
            if N <= 15: 
                col_labels = ['m', 'Axial Z', 'Radial X', 'Radial Y', 'Z Pos (μm)']
                cell_text = []
                for i in range(N):
                    pos_str = f"{raw_z[i] * 1e6:.3f}" if not trical_cache['trical_stable'] else f"{raw_z[i] * 1e6:.3f}"
                    cell_text.append([f"{i+1}", f"{freqs_z[i]:.4f}", f"{freqs_x[i]:.4f}", f"{freqs_y[i]:.4f}", pos_str])
                ...
                col_labels = ['m', 'Z', 'X', 'Y', 'Z(μm)', 'm', 'Z', 'X', 'Y', 'Z(μm)']
                cell_text = []
                half = math.ceil(N / 2)
                for i in range(half):
                    pos_str1 = f"{raw_z[i] * 1e6:.2f}"
                    row = [f"{i+1}", f"{freqs_z[i]:.2f}", f"{freqs_x[i]:.2f}", f"{freqs_y[i]:.2f}", pos_str1]
                    
                    idx2 = i + half
                    if idx2 < N:
                        pos_str2 = f"{raw_z[idx2] * 1e6:.2f}"
                        row.extend([f"{idx2+1}", f"{freqs_z[idx2]:.2f}", f"{freqs_x[idx2]:.2f}", f"{freqs_y[idx2]:.2f}", pos_str2])
                    else:
                        row.extend(["", "", "", "", ""])
                ...
                if mode <= half:
                    active_row = mode
                    for j in range(5): table[(active_row, j)].set_facecolor('gold')
                else:
                    active_row = mode - half
                    for j in range(5, 10): table[(active_row, j)].set_facecolor('gold')
                        
            trical_cache['table_drawn_for'] = current_state

        # --- UPDATE STATUS BAR ---
        if is_stable:
            status_text.set_text(f"STATUS: STABLE (1D Linear Chain) ✓ | Critical Radial w_r > {w_crit:.3f} MHz")
            status_text.set_color("darkgreen")
        else:
            status_text.set_text(f"STATUS: UNSTABLE (Buckles into 2D Zigzag) ✗ | Critical Radial w_r > {w_crit:.3f} MHz")
        ...
        for hl in highlights.values(): hl.set_data([], [])
        ...
        if not is_stable:
            fig.suptitle(f"2D Zigzag Threshold Crossed (Modes Invalid)", color='red', fontsize=14, y=0.98)
        else:
        ...
        if evecs is not None and mode <= evecs.shape[1]:
            ...
            ax_evec.set_title(f"Eigenvector $b_m^{{(j)}}$", fontweight='bold', fontsize=11)
            
            # Add numerical values beside each mode marker
            if N <= 15:
                for i, val in enumerate(vec):
                    y_offset = 0.12 if val >= 0 else -0.15
                    ax_evec.text(x_pos[i], val + y_offset, f"{val:.3f}", 
            ...
            if active_freq > 1e-6:
                ...
                zero_point_spread = np.sqrt(scipy_cst.hbar / (2 * MASS_ION * omega_m))
                eta_vec = vec * DELTA_K * zero_point_spread
            else:
                eta_vec = np.zeros_like(vec)

            # --- Rendering Lamb-Dicke Table ---
            ax_eta.set_title(r"Lamb-Dicke $\eta_m^{(j)}$", fontweight='bold', fontsize=11)
            ...
            table_eta = ax_eta.table(cellText=cell_text, colLabels=col_labels, loc='center', cellLoc='center')
...
# 5. Execute
# ==========================================
ani = animation.FuncAnimation(fig, animate, init_func=init, frames=1000, interval=100, blit=False)
plt.show()