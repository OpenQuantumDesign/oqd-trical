...
class QuantumOpticsBackend(BackendBase):
    ...
    Attributes:
        ...
        solver_options (Dict[str,Any]): QuantumOptics solver options
    ...
    def compile(self, circuit, fock_cutoff):
        ...
        Returns:
            experiment (QuantumOpticsExperiment): Compiled [`QuantumOpticsExperiment`][oqd_trical.backend.quantumoptics.interface.QuantumOpticsExperiment].
    ...
    def run(self, experiment, hilbert_space, *, project="."):
        ...
        Args:
            experiment (QuantumOpticsExperiment): [`QuantumOpticsExperiment`][oqd_trical.backend.quantumoptics.interface.QuantumOpticsExperiment] to be executed.
            hilbert_space (Dict[str, int]): Hilbert space of the system.
        ...
        states = states.reshape(
            -1,
            *reversed(hilbert_space.size.values()),
        )
        states = np.moveaxis(
            states,
            range(1, 1 + len(hilbert_space.size.keys())),
            range(-1, -1 - len(hilbert_space.size.keys()), -1),
        )
