...
from analog_Hamiltonian_AST import WaveCoefficient, OperatorScalarMul, KetBra, Displacement, Identity, OperatorKron, hc, OperatorMul
...
def extended_kron(op_list):
    '''
    Parameters:
    - op_list (List[Operator]): list of Operator objects as defined in analog_Hamiltonian_AST.py
    Returns:
    - extended_kron (OperatorKron): nested tree of tensored operators
    Mutates:
    - NOTHING
    '''
    if len(op_list) == 2:
...
def construct_H_tree(chamber, timescale):
    ...
    for m, laser in enumerate(lasers):
        for n, ion in enumerate(ions):
            ...
            for transition in ion.transitions:
                ...
                I = laser.I(0) # intensity generally some function of time to be evaluated at each time step
                ...
                int_coup_int_buffer[n] = KetBra(i = list(ion.state.keys()).index(lbl2), 
                                    j = list(ion.state.keys()).index(lbl1), dims = J_n)
                ...
                rabi_prefactor = WaveCoefficient(amplitude = Omega_nm/2,
                ...
                inner_ion_H = OperatorMul(op1 = OperatorScalarMul(coeff = rabi_prefactor, op = int_term), 
                                          op2 = mot_term)
    ...
    return Hamiltonian(tree = full_H + hc(), 
                       args = {'chamber': chamber, 'timescale': timescale},
                       N = len(ions), M = len(lasers), L = len(modes),
                       ion_N_levels = ion_N_levels, mode_cutoffs = mode_cutoffs)
