...
def time_evolve(H, psi_0, times, expt_ops = [], progress_bar = False):
    ...
    Returns:
    - Results object as defined in Results.py
    Mutates:
    - NOTHING
    '''
    ...
    out = mesolve(H.convert_to_qutip(), psi_0, times, e_ops = [op.qobj for op in expt_ops], args = {}, 
...
def zeeman_energy_shift(level, B):
    ...
    Returns:
    - Zeeman shift in energy of level in B
    Mutates:
    - NOTHING
...
def displace(ld_order, alpha, rwa_cutoff, Delta, nu, dims):
    ...
    Returns:
    - Time-dependent displacement operator (QobjEvo) that's dims x dims in shape
    Mutates:
    - NOTHING
    '''
